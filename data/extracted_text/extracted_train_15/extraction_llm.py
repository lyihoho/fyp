...
# Initialize your reinstalled offline engine client
local_client = ollama.Client()

# --- STEP 1: DETERMINISTIC BACKUP REGEX TOTAL LAYER ---
def deterministic_fallback_total(text):
    """
    If the model drops the total to 0.0 or misinterprets a layout string,
    this bottom-up calculator finds the true transaction total.
    """
    lines = text.split('\n')
...
if __name__ == "__main__":
    # Point directly to your extracted text folder assets
    ...
    try:
        ...
        print("\n[CRITICAL ERROR] Ollama background app is not running. Please open the Ollama application.")
        sys.exit(1)
        
    print("="*60)
    print("Pipeline Initiated. Running 100% Local Offline Parsing Step...")
    print("="*60)
    ...
    for filename in files_to_process:
        ...
        # Run text evaluation through local model instance
        ...
        # 1. Recover merchant name from top line strings if model misses it
        if not merchant or merchant.lower() in ["unknown", ""]:
            lines = [l.strip() for l in extracted_text.split('\n') if len(l.strip()) > 3]
            merchant = lines[0] if lines else "Unknown"

        # 2. Recover 0.0 layout errors cleanly via mathematical backup filters
    ...
    # Export out out your structured matrix target data sheet
